...
function kitLabelFromSingle(singleLabel){
  return makeKitLabel(singleLabel);
}
...
function inflationApplies(name, unitNorm){
  const lower = String(name||'').toLowerCase();
  const u = String(unitNorm||'').trim();
  return !(NO_INCREASE.has(lower) || u === 'mL' || u === 'pack');
}
...
// Only create kits when single stock >= 20 AND this item gets the price multiplier
const pr = PRICE_ROW_MAP.get(`${String(item.name).toLowerCase()}|||${String(sLabel).toLowerCase()}`);
const unitNorm = pr ? String(pr.unit||'').trim() : ((parseAmount(sLabel).unit||'mg').toLowerCase()==='iu' ? 'IU' : 'mg');
if(sStock >= KIT_MIN_SINGLE && inflationApplies(item.name, unitNorm)){
...
function computePrices(){
  ...
  for(const item of (GUIDE||[])){
    ...
    for(const sLabel of singles){
      ...
      const sStock = (item.stock && item.stock[sLabel]!=null) ? Number(item.stock[sLabel]) : 0;
      // Only create kits when single stock >= 20 AND this item gets the price multiplier
      const pr = PRICE_ROW_MAP.get(`${String(name).toLowerCase()}|||${String(sLabel).toLowerCase()}`);
      const unitNorm = pr ? String(pr.unit||'').trim() : ((parseAmount(sLabel).unit||'mg').toLowerCase()==='iu' ? 'IU' : 'mg');
      if(!(sStock >= KIT_MIN_SINGLE && inflationApplies(name, unitNorm))) continue;
...
async function loadData(){
  ...
  // Build lookup for kit gating (so we only create kits for items that get the price multiplier)
  PRICE_ROW_MAP = new Map();
  for(const r of PRICES_RAW){
    if(!r) continue;
    const [name, strength, basePrice, unit, stock] = r;
    PRICE_ROW_MAP.set(`${String(name).toLowerCase()}|||${String(strength).toLowerCase()}`, {basePrice:Number(basePrice)||0, unit:String(unit||'').trim(), stock:Number(stock)||0});
  }
...
function wireGuide(){
  ...
  $('#grid').addEventListener('click', (e)=>{
    ...
    if(doseBtn){
      // Kits should not open the mini calculator
      if(doseBtn.dataset.kind === 'kit') return;
