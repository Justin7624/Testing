...
// Price sheet default order should follow the catalog (Guide) order.
// Users can still click headers to sort, but the default is catalog order.
let sortKey = store.get('bvs.priceSortKey', 'catalog');
let sortDir = store.get('bvs.priceSortDir', 'asc');

// Map item name -> its order in the catalog (Guide)
  ...
  for(const item of filtered){
    ...
    const html = `
      <article class="card" data-key="${esc(item.key)}">
        ...
        <div class="dose-label" style="margin-top:12px">${esc(item.fact||'')}</div>
        <div class="dose-label" style="margin-top:8px">Available sizes:</div>
        ...
        <details ${guideExpanded?'open':''} class="more more-info">
...
function setGuideExpanded(v){
  ...
  $$('details.more-info').forEach(d => d.open = guideExpanded);
}
...
function computePrices(){
  ...
  return (PRICES_RAW||[]).map((row, rowIndex)=>{
    ...
    return {
      ...
      unit: unitNorm,
      skipInflation: skip,
      // For catalog-order sorting
      catalogIndex: CATALOG_ORDER.get(String(name).toLowerCase()) ?? 1e9,
      rowIndex
    };
  });
}
...
function renderPrices(){
  ...
  // Default catalog ordering: follow the Guide order (then preserve row order within an item).
  if(sortKey === 'catalog'){
    rows.sort((a,b)=>{
      if(a.catalogIndex !== b.catalogIndex) return a.catalogIndex - b.catalogIndex;
      // preserve original PRICES_RAW order for the same item name
  ...
  for(const r of rows){
    ...
    const tooltip = r.skipInflation
      ? `Raw ${money(r.priceBase)} (no multiplier)`
      : `Raw ${money(r.priceBase)} × ${PRICE_MULTIPLIER}`;
    tbody.insertAdjacentHTML('beforeend', `
      <tr>
        <td>${esc(r.name)}</td>
        <td>${esc(r.strength)}</td>
        <td class="num">${isFinite(r.amount)?num(r.amount):'—'}</td>
        <td class="num" title="${esc(tooltip)}">${money(r.price)}</td>
...
async function loadData(){
  ...
  // Build catalog order map from the Guide list order.
  CATALOG_ORDER = new Map((GUIDE||[]).map((it, idx)=>[String(it.name||'').toLowerCase(), idx]));
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // One-time migration: older versions defaulted to alphabetical name sorting.
  // Force back to catalog order unless the user explicitly re-sorts after this update.
  const migrated = store.get('bvs.priceSortMigrated', false);
  if(!migrated && (sortKey === 'name' || sortKey == null)){
    sortKey = 'catalog';
    sortDir = 'asc';
    store.set('bvs.priceSortKey', sortKey);
    store.set('bvs.priceSortDir', sortDir);
