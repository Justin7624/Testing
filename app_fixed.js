...
function inflationApplies(name, unit){
...
function renderGuide(){
  ...
  for(const item of filtered){
    ...
    const doses = (item.doses||[]).map(label=>{
      ...
      const badge = stock==null ? '' : `<span class="stock-badge ${cls}">${out?'Out of stock':`${stock} in stock`}</span>`;
      return `
    ...
    const more = item.more ? decodeEntities(item.more) : decodeEntities('<ul><li>Educational info only.</li></ul>');
            ...
            <div class="mini-body">
              <div class="mini-grid">
                ...
                <div class="mini-field">
                  ...
                  <div class="dose-row">
                    <input type="number" step="any" min="0" class="desired-input" data-role="desired" value="0" inputmode="decimal"/>
...
function openMini(card, doseLabel){
  ...
  desiredInput.value = "0"; // default desired dose to 0
  if(unit) {
...
function computePrices(){
  // Start from raw rows and derive kit rows.
  const raw = (PRICES_RAW||[]);
  const rows = [];

  // Lookup single-vial raw prices by (name|||strength)
  const singleRaw = new Map();
  for(const row of raw){
    const [name, strength, basePrice, unit, stock] = row;
    if(!isKitLabel(strength)){
      singleRaw.set(`${String(name)}|||${String(strength)}`, Number(basePrice)||0);
    }
  }

  // Keep all non-kit rows
  for(const row of raw){
    const [name, strength, basePrice, unit, stock] = row;
    if(!isKitLabel(strength)) rows.push([name, strength, basePrice, unit, stock]);
  }

  // Derive kit rows from GUIDE so they match the catalog
  for(const item of (GUIDE||[])){
    const name = item.name;
    for(const sLabel of (item.doses||[])){
      if(isKitLabel(sLabel)) continue;

      const sStock = (item.stock && item.stock[sLabel]!=null) ? Number(item.stock[sLabel]) : 0;
      if(!(sStock >= KIT_MIN_SINGLE)) continue;

      // Only for items/units that receive the normal price inflation multiplier
      const unitNorm = (parseAmount(sLabel).unit||'mg').toLowerCase()==='iu' ? 'IU' : 'mg';
      if(!inflationApplies(name, unitNorm)) continue;

      const kLabel = makeKitLabel(sLabel);
      if(!kLabel) continue;

      const kStock = kitStockFromSingle(sStock);
      const base = singleRaw.get(`${String(name)}|||${String(sLabel)}`);
      if(base == null) continue;

      rows.push([name, kLabel, Number(base)*KIT_PRICE_MULT, unitNorm, kStock]);
    }
  }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Decode ONLY if the string looks entity-encoded (e.g., &lt;ul&gt;).
  // If the string already contains real HTML tags, return as-is so it can render.
  if(str==null) return '';
  const s = String(str);
  if(!/[&](?:lt|gt|amp|quot|apos|#\d+|#x[0-9a-fA-F]+);/.test(s)) return s;
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // In case strings are HTML-entity encoded (e.g., &lt;ul&gt;)
  if(str==null) return '';
  const t = document.createElement('textarea');
