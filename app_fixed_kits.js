...
function renderGuide(){
  ...
  for(const item of filtered){
    ...
    // Build dose buttons:
    // - Show ALL single-vial options immediately.
    // - Show kit options AFTER singles (kit x10), derived from single-vial stock.
    ...
    for(const sLabel of singles){
      ...
      if(!kLabel) continue;
      // Keep kit visible if it used to exist OR if single stock > 20
    ...
    const doses = doseLabels.map(label=>{
      ...
      if(isKit){
        ...
        const matchSingle = singles.find(s=>{
          ...
          if(!isFinite(p.amount)) return false;
      ...
      const badge = stock==null ? '' : `<span class="stock-badge ${cls}">${out?'Out of stock':(stock<=5?`${stock} low stock`:`${stock} in stock`)}</span>`;
      return `
