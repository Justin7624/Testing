...
// Default to catalog order (same order as prices_raw in data.json)
let sortKey = store.get('bvs.priceSortKey', 'catalog');
let sortDir = store.get('bvs.priceSortDir', 'asc');

// One-time migration: older builds defaulted to name sort and persisted it.
// If we see that legacy value and we haven't migrated yet, reset to catalog.
try{
  const migrated = store.get('bvs.priceSortMigrated', false);
  if(!migrated){
    const prev = store.get('bvs.priceSortKey', null);
    if(prev === 'name'){
      sortKey = 'catalog';
      sortDir = 'asc';
    ...
    store.set('bvs.priceSortMigrated', true);
  }
}catch{}
...
function setTab(tab){
  ...
  // Ensure the price sheet is populated when switching tabs
  if(!isGuide){
    try{ renderPrices(); }catch(e){ console.error(e); }
  }
    ...
    const html = `
      <article class="card" data-key="${esc(item.key)}">
        ...
        <div class="mini-wrap" data-mini="${esc(item.key)}" aria-label="Mini calculator" style="display:none">
          <div class="mini-head">
            <div>
              <b>Peptide calculator</b>
              <div class="mini-meta" data-role="miniMeta"></div>
            </div>
        ...
        <details ${detailsOpen?'open':''} class="more">
          <summary class="dose-label">More info</summary>
...
function openMini(card, doseLabel, anchorBtn=null){
  ...
  // Move calculator directly under the clicked dose button (old-site behavior)
  if(anchorBtn && anchorBtn.parentElement){
    anchorBtn.insertAdjacentElement('afterend', mini);
  }else{
    // fallback: keep it parked after the doses block
    const parking = card.querySelector('[data-mini-parking]');
  ...
  // Meta line: "5 mg • 31 in stock" (if we can infer stock)
  if(metaEl){
    const stockBadge = anchorBtn?.querySelector('.stock-badge');
    let stockTxt = '';
    if(stockBadge){
      const m = stockBadge.textContent.match(/\b(\d+)\b/);
      if(m) stockTxt = `${m[1]} in stock`;
    }
    metaEl.textContent = stockTxt ? `${doseLabel} • ${stockTxt}` : doseLabel;
  }

  if(isFinite(amount)) amountInput.value = String(amount);
  if(vialUnitSel) vialUnitSel.value = (unit === 'iu') ? 'iu' : 'mg';

  // Autofill defaults per your spec
  if(diluentInput) diluentInput.value = '1';
  if(desiredInput) desiredInput.value = '0';
  if(desiredUnitSel){
    desiredUnitSel.value = (unit === 'iu') ? 'iu' : 'mg';
  }
...
function closeMini(mini){
  ...
  // Park back under the doses block so layout stays stable.
...
function computePrices(){
  ...
  // Extend with derived kit rows:
  //   - kit RAW price = 3x single RAW price
  //   - kit stock = floor((singleStock - 20) / 10)
  //   - if singleStock <= 20 -> kit stock = 0 (out of stock)

  const baseRows = (PRICES_RAW||[]).map((row, idx)=>{
    const [name, strength, basePrice, unit, stock] = row;
    return { _idx: idx, name, strength, basePrice, unit, stock };
  });

  const existingKitKeys = new Set(
    baseRows
      .filter(r => isKitLabel(r.strength))
      .map(r => `${String(r.name)}__${String(r.strength)}`)
  );

  const derivedKits = [];
  for(const r of baseRows){
    if(isKitLabel(r.strength)) continue; // derive from singles only
    const kitStrength = makeKitLabel(r.strength);
    if(!kitStrength) continue;

    const sStock = Number(r.stock)||0;
    const kStock = kitStockFromSingle(sStock);
    const kBase = (Number(r.basePrice)||0) * 3;
    const kKey = `${String(r.name)}__${String(kitStrength)}`;

    // Include when kits are available OR when the kit existed previously (show as OOS if needed).
    if(kStock > 0 || existingKitKeys.has(kKey)){
      derivedKits.push({
        _idx: r._idx + 0.1, // appears after its single in catalog order
        name: r.name,
  ...
  // Remove any existing kit rows from the base list so we don't duplicate or contradict.
  const noExistingKits = baseRows.filter(r => !isKitLabel(r.strength));

  const allRows = [...noExistingKits, ...derivedKits];
...
function renderPrices(){
  ...
  rows.sort((a,b)=>{
    ...
    if(sortKey === 'catalog'){
      return (Number(a._idx) - Number(b._idx)) * dir;
    }
    const ak = a[sortKey], bk = b[sortKey];
    if(typeof ak === 'string') return String(ak).localeCompare(String(bk)) * dir;
  ...
  for(const r of rows){
    ...
    tbody.insertAdjacentHTML('beforeend', `
      <tr>
        ...
        <td class="num" title="Raw ${money(r.priceRaw)} × ${Number(/*removed*/??1).toFixed(2)}">${money(r.price)}</td>
...
function wirePrices(){
  ...
  if(multInput){
    // initialize UI
    multInput.value = String(Number(/*removed*/ ?? 1).toFixed(2));
    multInput.addEventListener('input', ()=>{
      const v = Number(multInput.value);
      /*removed*/ = isFinite(v) ? v : 1;
      renderPrices();
    });
  }
